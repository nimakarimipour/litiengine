...
public abstract class MapObjectLoader implements IMapObjectLoader {
  ...
  protected boolean isMatchingType(IMapObject mapObject) {
    if (!Nullability.castToNonnull(mapObject.getType(), "reason...")
        .equalsIgnoreCase(this.getMapObjectType())) {
      log.log(
          Level.SEVERE,
  ...
  private static void loadCustomMapObjectProperties(IEntity entity, IMapObject mapObject) {
    for (final Field field :
      ...
      if (!ReflectionUtilities.setFieldValue(
          field.getDeclaringClass(), entity, field.getName(), value)) {
        log.warning(
            "entity #"
                + entity.getMapId()
                + ": value \""
  ...
  /**
   * If present, this method calls the private {@code afterTmxUnmarshal(IMapObject)} method on the
   * specified entity.
   *
   * <p>The {@link MapObjectLoader} implementation provides the possibility to extend the
   * unmarshalling of the IMapObject within the entity implementation by implementing a private
   * method with the name "afterTmxUnmarshal" accepting a parameter of type {@link IMapObject}. This
   * method is called, after the loading has been finished and the entities have been instantiated.
   *
   * @param entity The entity instance to call the "afterTmxUnmarshal" method on.
   * @param mapObject The map object to pass to the entity instance when invoking the
   *     "afterTmxUnmarshal" method.
   */
  private void callAfterTmxUnmarshal(IEntity entity, IMapObject mapObject) {
    Method afterTmxUnmarshal =
        ReflectionUtilities.getMethod("afterTmxUnmarshal", entity.getClass(), IMapObject.class);
    ...
    try {
      ...
      log.log(
          Level.SEVERE,
          "Could not invoke afterTmxUnmarshal method on type [{0}]: {1}",
