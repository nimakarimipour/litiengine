...
public final class TextRenderer {
     ...
     */
    public static void render(final Graphics2D g, final String text, Rectangle2D bounds, Align align, Valign valign, double offsetX, double offsetY, boolean scaleFont) {
        if (bounds == null) {
            return;
        }
        ...
        double locationY = bounds.getY() + valign.getLocation(bounds.getHeight(), getHeight(g, text)) + g.getFontMetrics().getAscent() + offsetY;
     ...
     * @see RenderingHints
     */
    public static void renderWithOutline(final Graphics2D g, final String text, final double x, final double y, double width, double height, @Nullable final Color outlineColor, final float stroke, Align align, Valign valign, final boolean antiAliasing) {
        if (text == null || text.isEmpty()) {
            return;
        }
