...
public class CustomPropertyProvider implements ICustomPropertyProvider {
    ...
    public CustomPropertyProvider() {
        // use Hashtable because it rejects null keys and null values
    ...
    @Override
    @Nullable
    public <T extends Enum<T>> T getEnumValue(String propertyName, Class<T> enumType, @Nullable T defaultValue) {
        ICustomProperty property = this.getProperty(propertyName);
        if (property == null) {
            return defaultValue;
        }
    ...
    @Override
    public void setValue(String propertyName, @Nullable Enum<?> value) {
        ICustomProperty property = createPropertyIfAbsent(propertyName);
        property.setType("string");
        property.setValue(value);
    }
    ...
    public List<String> getCommaSeparatedStringValues(String propertyName, @Nullable String defaultValue) {
        ...
        if (valuesStr != null && !valuesStr.isEmpty()) {
            for (String value : valuesStr.split(",")) if (value != null) {
                values.add(value);
            }
        }
        return values;
