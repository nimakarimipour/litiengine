 ...
 *
 * <p>A loop is an implementation that performs actions (e.g. physics, rendering, input processing,
 * ...) and updates other {@code IUpdatable} instances while the game is running.
 *
 * @see IUpdateable
 */
public interface ILoop extends ILaunchable {
  /**
   * Attaches the update method of the specified IUpdatable instance to be called every tick. The
   * tick rate can be configured in the client configuration and is independent from rendering.
   *
   * @param updatable The instance that will be registered for the update event.
   */
  ...
  /**
   * Returns a lock that can be used for actions that must be performed either within or
