 ...
 * events.
 *
 * <p>The time provided by this class is measured in (game loop) ticks which is essentially an
 * iteration of the game's main update loop.
 *
 * <p><b>Examples</b><br>
 * A common use-case is to track the passed time since a certain event occurred (e.g. some action
 * was performed by an {@code Entity}).<br>
...
public final class GameTime implements EnvironmentLoadedListener {
  ...
  /**
   * Calculates the delta time between the current game time and the specified ticks in
   * milliseconds.
   *
   * @param tick The tick for which to calculate the delta time.
   ...
   *
   * @param ticks The ticks that will be converted to milliseconds.
   ...
   *
   * @param ticks The ticks that will be converted to milliseconds.
   * @param updateRate The updateRate that is used for the conversion.
   * @return The milliseconds that correspond to the specified ticks.
   */
   ...
   *
   * @param milliseconds The milliseconds that will be converted to ticks.
