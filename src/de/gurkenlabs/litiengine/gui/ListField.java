...
public class ListField extends GuiComponent {
  ...
  public void refresh() {
    for (int column = 0; column < this.getNumberOfShownColumns(); column++) {
      for (int row = 0; row < this.getNumberOfShownRows(); row++) {
        ...
        /* Added null check for getListEntry(column, row) */
    ...
    /* Check if selectedComponent should be updated and is entry not null */
    if (!this.isEntireRowSelected()
      ...
      if (entry != null) {
        ...
        && this.selectionRow >= this.getVerticalLowerBound()
        && this.selectionRow < this.getVerticalLowerBound() + this.getNumberOfShownRows()) {
      ListEntry entry = this.getListEntry(0).get(this.selectionRow - this.getVerticalLowerBound());
      if (entry != null) {
        this.selectedComponent = entry;
      } else {
        this.selectedComponent = null;
  ...
  /**
   * If set to true, selecting a element will show a selection of the entire column on which that
  ...
  /**
   * If set to true, selecting a element will show a selection of the entire row on which that
