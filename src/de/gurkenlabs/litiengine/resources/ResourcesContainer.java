...
public abstract class ResourcesContainer<T> {
  ...
  public void add(@Nullable URL resourceName, T resource) {
    this.add(Nullability.castToNonnull(resourceName, "reason...").toString(), resource);
  }
   ...
   *
   * <p>Note that the name is <b>not case-sensitive</b>.
   *
   ...
   *     loading
   */
  public Future<T> getAsync(@Nullable String name) {
   ...
   *
   * <p>This method should be used, if it's not clear whether the resource is present on this
   * container.<br>
   * It is basically a combination of {@code get(String)} and {@code contains(String)} and allows to
   * check whether a resource is present while also fetching it from the container.
   *
